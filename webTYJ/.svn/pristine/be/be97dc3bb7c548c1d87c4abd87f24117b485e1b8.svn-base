...
define(function (require) {
    ...
    app.controller('personnelManagementSetCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        //查询出页面数据
        $scope.load = function(){
            $http.get(url + '/TeamPermissions/getCorePositionByProjectAndSpecialty2').success(function(data){
        ...
        $scope.searchManage = function(sid,pid,item,item2){
            if(sid==0){
            ...
            if(item2=="nothing"){
                layer.msg('此专业线未开启！',{icon:0,time:2000});
                return;
            }
