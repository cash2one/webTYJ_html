...
define(function (require) {
    ...
    app.controller('appUserCheckCtrl', ['$scope', '$http','$rootScope', '$location','$filter',function ($scope,$http,$rootScope,$location,$filter){

        var url = $rootScope.url;
        var fileUrl = $rootScope.fileUrl;    //上传的文件路径
        var filePath = '';
        $scope.Staff = {page: {}, staffName: ''};
        $scope.doClick(0);
        ...
        $scope.load = function () {
            var fetchFunction = function (page, callback) {
                ...
                //document.getElementById("stafName").value=document.getElementById("staffName").value;
                document.getElementById("stafNo").value=document.getElementById("staffNo").value;
                document.getElementById("sbirthday").value=document.getElementById("birthday").value;
                document.getElementById("stelphone").value=document.getElementById("telphone").value;
                document.getElementById("ssex").value=document.getElementById("sex1").value;
                document.getElementById("saddress").value=document.getElementById("address").value;
                document.getElementById("scardType").value=document.getElementById("cardType1").value;
                document.getElementById("scardNum").value=document.getElementById("cardNum").value;
            };
        ...
        $scope.showOptions=function(){
            //angular.element("#processStatus").val();
            //layer.alert(angular.element("#processStatus").val(),{icon : 0});
        ...
        $scope.changeState=function(){
            if($scope.search==0){
                ...
                $scope.search=0;
                angular.element("#staffNo").val("");
                angular.element("#telphone").val("");
                //$("#nativePlace").val("");
                angular.element("#address").val("");
                angular.element("#sex1").val("");
            }
        }**/
        ...
        $http.get(url + '/Project/getProjectByState').success(function(data){
            ...
            //得到在属性配置中的属性
            //学历
            $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/eduLever/'+ projectId).success(function(data){
                $scope.eduLeverList = data.DataDictionarySlave;
                var da=$scope.eduLeverList.length;
            });
            //婚姻状况
            ...
            //查询所有的客户信息
            $scope.staffList={};
            $http.get(url+'/staff/listAllStaff').success(function(data){
                $scope.staffList=data.Staff;
                for(var i=0;i<$scope.staffList.length;i++){

                    if($scope.staffList[i].cardNum.length>18){
                        $scope.staffList[i].cardNum=$scope.staffList[i].cardNum.substr(0, $scope.staffList[i].cardNum.indexOf('S'));
                    }
                }
            });
        ...
        $scope.checkIdNumber=function(IdNumber){
            ...
            if(IdNumber==null||IdNumber.length==0||IdNumber==undefined){
                layer.alert('身份证号码不能为空！',{icon : 0});
                flag=1;
                return;
            }else{
        ...
        $scope.toUpdate = function () {
            ...
            if($scope.showStatus==0){
                ...
                if ($scope.chooseData.datas.length > 1) {
                    ...
                    $scope.staff1 =$scope.centerChange($scope.chooseData.datas[0]);
                    //$scope.staff1.birthday = new Date($scope.staff1.birthday;
                    $scope.staff1.birthday = new Date($scope.staff1.birthday);
                    angular.element("#birthdayEdit").val($scope.staff1.birthday);

                    /*身份证大于18位时,取出来需要去掉最后为字母*/
                    if($scope.staff1.cardNum.length>18){
                        $scope.staff1.cardNum=$scope.staff1.cardNum.substr(0, $scope.staff1.cardNum.indexOf('S'));

                    }else{
                        $scope.staff1.cardNum=$scope.staff1.cardNum;
                    }
                    $scope.staff1.oldcardNum=$scope.staff1.cardNum;
                    ...
                    angular.element('#update').modal({backdrop: 'static', keyboard: false});
                    //文件上传
                    ...
                    angular.element(function(){
                        ...
                        angular.element("#zyupload").zyUpload({
                            ...
                            onSuccess: function(file, response){          // 文件上传成功的回调方法
                                ...
                                $scope.staff1.annexs.push($scope.annex);
       ...
       /* $scope.saveCity = function () {
            if ($scope.staff1.province != undefined || $scope.staff1.city != undefined || $scope.staff1.district != undefined) {
                $scope.staff1.nativePlace = $scope.staff1.province + $scope.staff1.city + $scope.staff1.district;
            }
        ...
        $scope.changeData=function(){
            var cardType = angular.element("#cardType").val();
            if(cardType=="number:1"){
                $scope.sf=false;
                $scope.hz=true;
            }
           /* else{
                //$scope.staff1.cardNum='';
                $scope.sf=true;
                $scope.hz=false;
            }*/
        ...
        $scope.checkValue=function(item){
            flag=0;
            if(item.staffNo ==null||item.staffNo==""){
                layer.alert('员工工号不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.staffName ==null||item.staffName==""){
                layer.alert('姓名不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.sex ==null){
                layer.alert('员工性别不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.telphone ==null||item.telphone==""){
                ...
                layer.alert('出生日期不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.telphone!=null&&item.telphone!=""){
                $scope.checkPhone(item.telphone);
            }
            if(item.cardType=="0"){
            ...
            if($scope.staff1.oldcardNum==item.cardNum){

            }else{
                if($scope.staffList.length>0){
                    for(var i=0;i<$scope.staffList.length;i++){
                        if(item.staffId!=$scope.staffList[i].staffId){
                            if(item.cardNum==$scope.staffList[i].cardNum){
        ...
        $scope.save = function () {
            ...
            $scope.checkValue($scope.staff1);
            $scope.staff1.birthday =  new Date($scope.staff1.birthday);
            if(flag!=0)
                return;
            $scope.staff1.cardNum = $scope.staff1.cardNum;
            $http.put(url + '/staff/UpdateStaff', {Staff: $scope.staff1}).success(function () {
            ...
            if($scope.showStatus==0){
                ...
                if ($scope.chooseData.datas.length >= 1) {
                    layer.confirm("您确定要删除选中员工吗？", {btn: ['是', '否']}, function () {
                        ...
                        $http.put(url + '/staff/updateStaffStateById', {Staff: $scope.dleteData}).success(function () {
        ...
        $scope.getData = function(item)
        {
            ...
            {
                if(item.staffId == allData[i].staffId)
                {
                    if(document.getElementById(item.staffId).checked)
                    {
                        document.getElementById(item.staffId).checked = false;
                        document.getElementById(item.staffId).parentNode.parentNode.style.background = '';
                        var temp = $scope.chooseData.datas;
                        $scope.chooseData = {datas: []};
                        for (var i = 0; i < temp.length; i++) {
                            if (temp[i].staffId != item.staffId) {
                    ...
                    {
                        document.getElementById(item.staffId).checked = true;
                        document.getElementById(item.staffId).parentNode.parentNode.style.background = '#f6f8fa';
                        $scope.chooseData.datas.push(item);
                        break;
                    }
                }
            ...
            for(var i=0;i<allData.length;i++)
            {
                if(document.getElementById(allData[i].staffId).checked)
                {
                    num.push(i);
        ...
        function allcheck(datastaffList)
        {
            if( $scope.chosestate=='0'){
                ...
                for(var i=0;i<datastaffList.length;i++){
                    document.getElementById(datastaffList[i].staffId).checked = true;
                    $scope.chooseData.datas.push(datastaffList[i]);
                    document.getElementById(datastaffList[i].staffId).parentNode.parentNode.style.background = '#f6f8fa';
                }
                document.getElementById("allchose").checked = true;
                $scope.chosestate = '1';
            }else{
                for(var i=0;i<datastaffList.length;i++){
                    document.getElementById(datastaffList[i].staffId).checked = false;
                    document.getElementById(datastaffList[i].staffId).parentNode.parentNode.style.background = '';
                }
                $scope.chosestate = '0';
                $scope.chooseData = {datas:[]};
                document.getElementById("allchose").checked = false;
        ...
        /**
         *  enter 查询  2016/3/9 By yeshengqiang 开始
        ...
        /**
         *  enter 查询  2016/3/9 By yeshengqiang 结束
         */
