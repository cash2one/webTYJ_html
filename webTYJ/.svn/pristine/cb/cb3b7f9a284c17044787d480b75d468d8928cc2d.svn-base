...
define(function (require) {
    ...
    app.controller('acceptanceOrdersCtrl', ['$scope', '$http','$rootScope','$stateParams','$location', function ($scope,$http,$rootScope,$stateParams,$location) {
        ...
        $scope.addSave=function(){
            ...
            $scope.currentEditArea={};
            $scope.editArea=false;
            $scope.RenovationAcceptanceOrders.renovationAcceptanceRecords = $scope.currentBuilding.areaNew;
        };
        /**
         * 新增取消
         */
...
//获取任务催促次数
        ...
        $http.get(url +'/TasksJournal/getTasksJournalbyTasksId/'+$scope.TasksJournal.taskId).success(function(data) {
            //alert("查询成功！");
            //console.log(data);
            $location.path("/index/serviceRequest/decorationManagement/acceptanceOrders/"+$scope.TasksJournal.taskId).search({decorationWorkId:$scope.TasksJournal.taskId});
        ...
        $scope.item1=function(item){
            ...
            $http.get(url +'/TasksJournal/getTasksJournalbyDateandType/'+ item.creOrderType+"/"+item.operationTime).success(function(data) {
                ...
                //装修核查单关闭显示
                if($scope.aa.renovationAcceptanceOrders!=null){
                    if($scope.aa.renovationAcceptanceOrders.orderState==0){
                        $scope.closed=true;
                    }else{
                        $scope.closed=false;
                    }
        ...
        $scope.RenovationAcceptanceOrders.letterRecords = $scope.currentLetter;
        $scope.RenovationAcceptanceOrders.taskId=$stateParams.decorationWorkId;
        $scope.RenovationAcceptanceOrders.renovationAcceptanceRecords = $scope.currentBuilding.areaNew;
        $scope.addCheckOrders = function(taskId){
            if($scope.Tasks.taskId!=0 || $scope.Tasks.taskId!=null){
                $http.post(url+'/RenovationAcceptanceOrders/insertRenovationAcceptanceOrders',{RenovationAcceptanceOrders:$scope.RenovationAcceptanceOrders}).success(function(){
                    $scope.RenovationAcceptanceOrders.taskId=$stateParams.decorationWorkId;
                    $location.path("/index/serviceRequest/decorationManagement/acceptanceOrders/"+$scope.RenovationAcceptanceOrders.taskId).search({decorationWorkId:$scope.RenovationAcceptanceOrders.taskId});
                    $scope.load();
                }).error(function(data,status,headers,config){
                    alert("新增失败！");
                });
        ...
        $scope.CloseComplaintOrders=function(){
            ...
            $scope.aa.renovationAcceptanceOrders.closePersonId=16;
            $http.put(url+"/RenovationAcceptanceOrders/closeRenovationAcceptanceOrders",{RenovationAcceptanceOrders: $scope.aa.renovationAcceptanceOrders}).success(function(){
                //alert("关闭投诉状态成功！");
                console.log($scope.aa.renovationAcceptanceOrders.orderState);
                $scope.closed=true;
            }).error(function(data,status,headers,config){
                alert("关闭投诉状态失败！");
            });
            $scope.aa.renovationAcceptanceOrders={}
        };
