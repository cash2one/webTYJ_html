...
define(function (require) {
    ...
    app.controller('bankCollectionCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        //校验合同号
        //var _contactNumber = JSON.parse(localStorage.getItem("contract_number"));
        //var c=$scope.collections.contractNumber;
        $scope.checkCn = function() {
            //alert("合同号查询失败");
            var co=$scope.collectionManage.contractNumber;
            $http.get(url + '/CollectionManage/checkContactNumber/'+co).success(function (data) {
                    if (data.result == "1") {
                        layer.alert("合同号重复!", {icon: 0, time: 2000});
                    }
            }).error(function (data){
