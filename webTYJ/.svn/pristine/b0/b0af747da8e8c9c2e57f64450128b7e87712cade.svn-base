...
define(function (require) {
    ...
    app.controller('meterReadingSRCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.ServiceRequest = {page:{},tasks:{addressIds:[]}};
        ...
        $scope.detail1 = function(){
            $scope.ServiceRequest.tasks= {addressIds:[]};
            $scope.mark = true;
            $scope.btnIndex = null;
        };
        $scope.detail2 = function(index){
            ...
            if($scope.ServiceRequest.tasks.addressIds.length>1){
                $scope.addWaterCheckFullname=$scope.ServiceRequest.tasks.addressIds.length+'个地址';
            }
        ...
        $scope.save=function(){
           ...
           /* var fullname = app.get("valueCheck").isNull($scope.addWaterCheck[0].fullname);
            ...
            if($scope.ServiceRequest.tasks.addressIds==null|| $scope.ServiceRequest.tasks.addressIds.length==0){
                layer.msg('请选择地址！',{icon : 0,time : 2000});
                return;
            }
            //$scope.ServiceRequest.tasks.addressId = $scope.addWaterCheck[0].id;
            //负责人
            if($scope.ServiceRequest.tasks.staff==null){
            ...
            $scope.ServiceRequest.tasks.taskType = 16;
        ...
        $scope.addReqService=function(customerId){
            ...
            if(isNullTask==true){//当任务不是空的时候
                ...
                layer.msg('任务不能为空！',{icon : 0});
                return;
            }
            if($scope.ServiceRequest.remarks==null||$scope.ServiceRequest.remarks==''){
                layer.msg('服务请求不能为空！',{icon : 0});
                return;
            }
            //为服务请求创建用户id和地址id
        ...
        $scope.addWaterReading = function(){
            ...
            if($scope.addWaterCheck.length==0){
                return
            }else if($scope.addWaterCheck.length==1){
                $scope.addWaterCheckFullname = $scope.$scope.addWaterCheck[0].Fullname;
            }else if($scope.addWaterCheck.length>1){
                $scope.addWaterCheckFullname = $scope.addWaterCheck.length+'个地址';
            }
            for(var i=0;i<$scope.treeResult.treeList.length; i++){
                $scope.ServiceRequest.tasks.addressIds.push($scope.treeResult.treeList[i].id);
            }
        ...
        $scope.addPerson=function(){
            ...
            console.log($scope.ServiceRequest.tasks);
        };
