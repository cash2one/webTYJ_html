...
define(function (require) {
    ...
    app.controller('accountManagementsCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
         ...
         */
        $scope.bill ={};
        $scope.showInquiry = function(){
            if(($scope.searchAssetAccount.name == null || $scope.searchAssetAccount.name=="")
                && ($scope.searchAssetAccount.cardNum == null || $scope.searchAssetAccount.cardNum == "")
                && ($scope.searchAssetAccount.houseAddress == null || $scope.searchAssetAccount.houseAddress == "")){
                layer.msg('查询条件不能为空!',{icon : 0,time : 1000});
                return;
            }
            $scope.bill.custName=$scope.searchAssetAccount.name;
            $scope.bill.cardNum=$scope.searchAssetAccount.cardNum;
            $scope.bill.fullName=$scope.searchAssetAccount.houseAddress;
            var bill=JSON.stringify($scope.bill);
            sessionStorage.setItem("bill",bill);
            //更改了路由
            $location.path("/index/cashier/cashier/billInquiry");
        }
        ...
        $scope.searchData = function(){
            var s=$scope.searchAssetAccount;
            if((s.name==null|| s.name=='')&&(s.cardNum==null|| s.cardNum=='')&&(s.houseAddress==null|| s.houseAddress=='')&&(s.assetAccountType==undefined))
            {
         ...
         * 界面跳转取值
         */
        var searchAssetAccount = sessionStorage.getItem("searchAssetAccount");
        $scope.searchAssetAccount = JSON.parse(searchAssetAccount);
        if($scope.searchAssetAccount){
            $scope.searchData();
        }
         ...
         */
        $scope.chargeType={};
        $scope.chargeType.projectId=$scope.projectId;
        ...
        $scope.searchAsset=function(item){
            ...
            $http.post(url + "/AssetAccountDetail/getAssetAccountDetailById",{AssetAccount:item}).success(function(data){
