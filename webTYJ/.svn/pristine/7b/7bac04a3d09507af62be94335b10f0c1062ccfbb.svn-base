    ...
    app.controller('projectTeamManagementCtrl', ['$scope', '$http', '$rootScope',
        function ($scope, $http, $rootScope) {
            ...
            $http.get(url + '/Teamwork/listAllTeamwork').success(function(data){
                $scope.teamworkStaffAdd = data.Teamwork;
                console.log($scope.teamworkStaffAdd);
            });
            ...
            $scope.teamworkstaffAddList
            $scope.clickTeamwork = function(){
                $http.get(url+'/teamworkStaffAdd/listTeamworkStaffAdd/'+$scope.tUser.employId).success(function(data){
                    $scope.teamworkstaffAddList=data.TeamworkStaffAdd;
                    for(var i=0; i<$scope.teamworkStaffAdd.length; i++){
                        for(var j=0; j<$scope.teamworkstaffAddList.length; j++){
                            if($scope.teamworkStaffAdd[i].id == $scope.teamworkstaffAddList[j].teamworkId){
                                document.getElementById($scope.teamworkStaffAdd[i].id).checked = true;
                                document.getElementById($scope.teamworkStaffAdd[i].id).parentNode.parentNode.style.background = '#f6f8fa';
                            }
                        }
                    }
                    angular.element("#configTeamModal").modal("show");
                }).error(function(data){
                    layer.alert('获取可加入团队失败！', {icon: 2});
                });
            }
            ...
            $scope.clickConfigList = function (item) {
                if(!document.getElementById(item.id).checked)
                {
                    ...
                    $http.get(url+"/teamworkStaffAdd/delTeamworkStaff/"+item.id+'/'+$scope.tUser.employId).success(function(data){
                ...
                }else
                {
                    ...
                    $scope.teamworkstaffAdd = {};
                    $scope.teamworkstaffAdd.teamworkId = item.id;
                    $scope.teamworkstaffAdd.staffId = $scope.tUser.employId;
                    $http.post(url + '/teamworkStaffAdd/insertTeamworkStaff', {TeamworkStaffAdd: $scope.teamworkstaffAdd}).success(function (data) {
...
/*                var tmp = 0
            ...
            $scope.cancelOperator=function(){
                layer.confirm('是否取消团队操作员？', {
                    btn: ['是', '否']
                }, function () {
                    $http.get(url+'/Admin/delHrAuthority/'+$scope.tUser.employId).success(function(data){
            ...
            $scope.delMember = function (items, index) {
                ...
                $scope.insertStaff.markState = items.markState;
