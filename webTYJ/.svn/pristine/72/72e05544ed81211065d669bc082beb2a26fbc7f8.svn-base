        ...
        $scope.addProduct = function (){
            if(angular.isUndefined($scope.Product)){
               ...
               /* var pEffect = app.get('valueCheck').isNull($scope.Product.effectDate);
                ...
                var pEffect = app.get('valueCheck').isNull($scope.AccessCard.productBeginDate);
                if(!pEffect.state){
                    layer.msg('请选择启用时间',{icon : 0,time : 1000});
                    return;
                }
                var bdate=$scope.AccessCard.productBeginDate;
                var edate=$scope.AccessCard.productEndDate;
