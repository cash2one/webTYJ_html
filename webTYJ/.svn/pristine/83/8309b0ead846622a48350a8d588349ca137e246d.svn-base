...
var ZYFILE = {
    ...
    funDealtFiles: function() {
        ...
        var typeUp =$("#excelTyle").val();
        if (typeUp == 1) {
            for (var i = 0; i < this.uploadFile.length; i++) {
                if (this.uploadFile[i].type == 'application/vnd.ms-excel' || this.uploadFile[i].type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
                } else {
    ...
    $.fn.zyUpload = function(options, param) {
        ...
        return this.each(function() {
            ...
            this.funDisposePreviewHtml = function(file, e) {
                ...
                var newStr = file.name.split("").reverse().join("");
                var type = newStr.split(".")[0].split("").reverse().join("");
                var fileImgSrc = "images/upload/skins/images/fileType/";
                if (type == "rar") {
