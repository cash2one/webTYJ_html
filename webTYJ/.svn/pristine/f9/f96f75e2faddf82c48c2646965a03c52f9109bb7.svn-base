...
define(function (require) {
   ...
   */
        ...
        //通用
        $http.get(url + "/Tasks/getTaskPoolsCountBytaskType/21").success(function(data){
            $scope.countGeneral = data.info;
        }).error(function(data,status,headers,config){
            layer.msg("获取维修个数失败!",{icon : 0,time : 2000})
            return;
        ...
        //重点客户申请
        $scope.theme='重点客户申请';
        $http.get(url + "/PersonEmphasis/getPersonEmphasisSum/" + $scope.theme).success(function(data){
            $scope.countPE = data.info;
            console.log($scope.countPE);
        }).error(function(data,status,headers,config){
            layer.msg("重点客户申请个数失败!",{icon : 0,time : 2000})
            return;

        });
        //重点客户失效申请
        $scope.theme1='重点失效客户申请';
        $http.get(url + "/PersonEmphasis/getFailPersonEmphasisSum/" + $scope.theme1).success(function(data){
            $scope.countFPE = data.info;
            console.log($scope.countFPE);
        }).error(function(data,status,headers,config){
            layer.msg("重点失效客户申请个数失败!",{icon : 0,time : 2000})
            return;
        });
        //vip客户申请
        $scope.theme2='vip客户申请';
        $http.get(url + "/PersonVip/getPersonVipSum/" + $scope.theme2).success(function(data){
            $scope.countPvs = data.info;
            console.log($scope.countPvs);
        }).error(function(data,status,headers,config){
            layer.msg("vip客户申请申请个数失败!",{icon : 0,time : 2000})
            return;

        });
        //重点客户失效申请
        $scope.theme3='vip客户失效申请';
        $http.get(url + "/PersonVip/getFailPersonVipSum/" + $scope.theme3).success(function(data){
            $scope.countFPvs = data.info;
            console.log($scope.countFPvs);
        }).error(function(data,status,headers,config){
            layer.msg("vip客户失效申请个数失败!",{icon : 0,time : 2000})
            return;
        });
