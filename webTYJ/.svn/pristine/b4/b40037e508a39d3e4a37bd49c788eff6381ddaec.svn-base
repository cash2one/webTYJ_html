...
define(function (require) {
    var app = require('../../../app');
    app.controller('unionPayCollectionCtrl', ['$scope', '$http', '$rootScope', '$location', '$filter', '$timeout', function ($scope, $http, $rootScope, $location, $filter, $timeout) {
        ...
        $scope.batchNumber = "";
        $("#pid").text($scope.project.projectName);
        $("#pid").click(function () {
        ...
        //$scope.select1($scope.searchPaginator1.object.unionpayCounterofferTotals[1]);
        /**
         * 默认选中第一行数据并且显示对应的明细数据
         */
        var i = 0;
        var time = setInterval(function () {
            ...
            if ($scope.searchPaginator.object.unionpayCollectionTotalls != undefined) {
                if ($scope.searchPaginator.object.unionpayCollectionTotalls.length > 0) {
                    $scope.select($scope.searchPaginator.object.unionpayCollectionTotalls[1]);
                    clearInterval(time);
                }
            }
        }, 1000);
        ...
        var time1 = setInterval(function () {
            ...
            if ($scope.searchPaginator1.object.unionpayCounterofferTotals != undefined) {
                if ($scope.searchPaginator1.object.unionpayCounterofferTotals.length > 0) {
                    $scope.select1($scope.searchPaginator1.object.unionpayCounterofferTotals[1]);
                    clearInterval(time1);
                }
            }
        ...
        $scope.load1 = function () {
            ...
            var fetchFunction1 = function (page, callback) {
                ...
                $scope.unionpayCounterofferTotal_check.projectId = projectId;
                $scope.unionpayCounterofferTotal_check.offerState = 1;
                $scope.show4 = false;
            ...
            /*
             $http.post(url + '/UnionpayCounterofferTotal/listPageUnionpayCounterofferTotal', {UnionpayCounterofferTotal: $scope.unionpayCounterofferTotal_check}).success(function (data) {
             if (data.PageRestful.unionpayCounterofferTotals[1] != null) {
             $scope.select1(data.PageRestful.unionpayCounterofferTotals[1]);
             }
             });*/
        };
        $scope.load1();
        ...
        $scope.select1 = function (item) {
            ...
            var unionDetail = function (page, callback) {
                ...
                $scope.unionpayCollectionDetail.unionpayCounterofferTotalId = item.unionpayCounterofferCotalId;
        ...
        $scope.newCollection = function () {
            $http.get(url + '/UnionpayCollectionTotall/newCollection/' + projectId).success(function (data) {
        ...
        $scope.exportNewCollectionFile = function () {
            layer.confirm("是否导出托收文件？",
                {btn: ['是', '否']}, function () {
                    ...
                    $http.get(url + '/UnionpayCollectionTotall/updateCollectionState/' + $scope.uctId).success(function(data){
                        layer.msg("导出托收文件成功 ", {icon: 0});
                        $scope.finance();
                        $scope.load();
                        var i = 0;
        ...
        $scope.cancelCollection = function () {
            $http.get(url + '/UnionpayCollectionTotall/cancelCollection/' + projectId);
        ...
        $scope.getOfferTotals = function (batchNumber) {
            ...
            var fetchFunction = function (page, callback) {
                ...
                $scope.uctotal.projectId = projectId;
            ...
            $http.post(url + '/UnionpayCounterofferTotal/listPageUnionpayCounterofferTotal', {UnionpayCounterofferTotal: $scope.uctotal}).success(function (data) {
                ...
                if (uct != null) {
                    $scope.getOfferDatils(data.PageRestful.unionpayCounterofferTotals[1].unionpayCounterofferCotalId);
                }
            });
        }
