...
define(function (require) {
    ...
    app.controller('requestCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.checkPersonEmphasis=function(personEmphasis){
            if(personEmphasis.applystype==2){
                if(personEmphasis.applystatus==0){
                    ...
                    layer.alert('该客户已提交重点客户申请,且申请未通过！',{icon : 0});
                    $scope.cancleAll();
                    $location.path("/index/propertyService/staffHome/importantClients/request");
                }
        ...
        $scope.saveAll=function(){
            ...
            $http.get(url + '/PersonEmphasis/getPersonEmphasisState/'+$scope.id).success(function(data) {
                ...
                $http.get(url + '/PersonVip/getPersonVipState/'+$scope.id).success(function(data) {
                    ...
                    $scope.personEmphasis.theme="重点客户申请";
                    ...
                    $scope.personEmphasis.applystype=2;//申请
                    $scope.personEmphasis.applystatus=0;//审核状态
                    $scope.personEmphasis.applyfilepath=$scope.applyfilepath;
                    if($scope.personEmphasis.applyreason==null||$scope.personEmphasis.applyreason==""){
