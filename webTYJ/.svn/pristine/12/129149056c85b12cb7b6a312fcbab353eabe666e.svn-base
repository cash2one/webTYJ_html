...
define(function (require) {
    ...
    app.controller('newCustomersCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.selectBox=function (item){
            for(var i=1;i<$scope.searchPaginator.object.personCustNew.length;i++){
                if(item.custId==$scope.searchPaginator.object.personCustNew[i].custId){
                    ...
                    $scope.personCustS.btnIndex=$scope.searchPaginator.object.personCustNew[i].custId;
                    $scope.personCustS.urgentContactPerson=$scope.searchPaginator.object.personCustNew[i].name;
                    $scope.personCustS.urgentContactPhone=$scope.searchPaginator.object.personCustNew[i].registerPhone;
                    $scope.btnIndex =$scope.searchPaginator.object.personCustNew[i].custId;
                    //$scope.personCustNew.urgentContactPerson=$scope.searchPaginator.object.personCustNew[i].name;
                    //$scope.personCustNew.urgentContactPhone=$scope.searchPaginator.object.personCustNew[i].registerPhone;
                }else{
        ...
        $scope.checkValue=function(item){
            ...
            if(item.name ==null||item.name==''){
                ...
                $scope.checkPhone(item.registerPhone);//注册电话
                ...
                if($scope.personCustNewList.length>0){
                //    var cardNo='';
                // if(item.cardNum.length>=18){
                //     cardNo=item.cardNum+'s';
                // }else{
                //     cardNo=item.cardNum;
                // }

                    for(var i=0;i<$scope.personCustNewList.length;i++){



                        //if(cardNo==$scope.personCustNewList[i].cardNum){
                        //    layer.alert('证件号码已存在',{icon : 0});
                        //    flag=1;
                        //    break;
                        //}

                        if(item.registerPhone==$scope.personCustNewList[i].registerPhone){
                            layer.alert('注册号码已存在',{icon : 0});
                            flag=1;
                            break;
                        }
                    }
        ...
        $scope.insertPerson=function(backHide){
            $scope.checkValue($scope.personCustNew);
            console.log($scope.personCustNew);
            if(flag!=0)
                return;
            $http.post(url+'/PersonCustNew/insertPersonCustNewRestful',{PersonCustNew:$scope.personCustNew}).success(function(data){
               // layer.msg(data.Info.info.$,{icon : 1,time : 2000});
                layer.alert(data.Info.info.$,{icon : 0});
               if(data.Info.info.$=='添加成功') {
                   if(backHide=='1'){
                       $('#addUrgentSimple').modal('hide');
                       $scope.load();
                   }else {
                       $location.path("/index/baseManagement/project/queryCustomers");
                   }
                }
     ...
     //536 【客户问题】个人客户紧急联系人问题
        //新增紧急联系人列表
        $scope.urgentPerson=function(){
            $scope.projects={};
            $scope.personCustNews={page:{}};
            $scope.load=function(){
            ...
            //设置数据默认以列表显示
            $scope.showStatus=1;
            angular.element('#choosePerson').modal({backdrop: 'static', keyboard: false});
        }

        $scope.addUrgentSimple=function (){
            angular.element('#addUrgentSimple').modal({backdrop: 'static', keyboard: false});
        }
        //新增紧急联系人
        $scope.getdata=function(items) {
            ...
            for(var i=0;i<chk.length;i++){
                if( chk[i].checked==true){
                    ...
                    $scope.personCustS.urgentContactPerson=$scope.person.name;
                    $scope.personCustS.urgentContactPhone=$scope.person.registerPhone;
                    //$scope.personCustNew.urgentContactPerson=$scope.person.name;
                    //$scope.personCustNew.urgentContactPhone=$scope.person.registerPhone;

                    if(items.cardNum.length>18){
                        $scope.cardNum=items.cardNum.substr(0, items.cardNum.indexOf('S'));
                    }else{
                        $scope.cardNum=items.cardNum;
                    }
                    $scope.person.birthday = new Date($scope.person.birthday);
                    $scope.personCustS.btnIndex=items.custId;
            ...
            //    $scope.personCustNew.urgentContactPerson=personInfo[1];
            //    $scope.personCustNew.urgentContactPhone=personInfo[2];
            //
            //});
        ...
        $scope.saveNewPerson=function (){
            $scope.btnIndex =$scope.personCustS.btnIndex;
            $scope.personCustNew.urgentContactPerson=$scope.personCustS.urgentContactPerson;
            $scope.personCustNew.urgentContactPhone=$scope.personCustS.urgentContactPhone;
            angular.element('#choosePerson').modal('hide');
        }
        //紧急联系人取消
        <!--bug 1620 陈秋旭-->
        ...
        $scope.addCity=function(){
            angular.element('#chooseCity').modal({backdrop: 'static', keyboard: false});
        }


/*        //精拍仪
        $scope.instrument=function() {
            layer.alert('功能暂未开通！',{icon : 0});
        }*/

        //籍贯取消
        $scope.cancleCity=function(){
            $scope.personCustNew.nativePlace='';
            angular.element('#chooseCity').modal('hide');
        }

        //籍贯保存
        $scope.saveCity=function(){
            var nativeLength=$scope.personCustNew.province +  $scope.personCustNew.city +  $scope.personCustNew.district;
           if(typeof($scope.personCustNew.province) == "undefined"){
               $scope.personCustNew.nativePlace ='';
           }
              else{
                    $scope.personCustNew.nativePlace = $scope.personCustNew.province + $scope.personCustNew.city + $scope.personCustNew.district;
                }
            angular.element('#chooseCity').modal('hide');
        };
        ...
        $scope.changeDataUrgent=function(){
            //  console.log($scope.addStaff);
            var cardType = $("#cardTypeUrgent").val();
            if(cardType=="number:1"){
                $scope.sf=false;
                $scope.hz=true;
            }else{
        ...
        /*$scope.identification=function() {
            //ReadIDCARDInfo();
            //layer.alert(VIDEOCAP.Get_IDName(),{icon : 0});
            //var reader = $('#VIDEOCAP');

            //layer.alert(VIDEOCAP.Get_IDName(),{icon : 0});
            $scope.personCustNew.personName = reader.Get_IDName();
        };*/
        ...
        $scope.ReadIDCARDInfo = function(index) {
            ...
            //VIDEOCAP.GetColorSpaceOutputsizeNum(0);
            ...
            $scope.personCustNew.cardNum = VIDEOCAP.Get_IDNumber();//身份证号码

            $scope.personCustNew.name = VIDEOCAP.Get_IDName();//姓名

            if(index==1){
                $scope.personCustNew.cardType = 0;
            }else{
                $scope.personCustNew.cardType = 1;
            }

            if(VIDEOCAP.Get_Sex()=='男'){
                $scope.personCustNew.sex = 0;
            }else{
                $scope.personCustNew.sex = 1;
            }
            //$scope.personCustNew.sex = VIDEOCAP.Get_Sex();

            $scope.personCustNew.nation = VIDEOCAP.Get_Nation()+'族';//民族

            $scope.personCustNew.birthday = VIDEOCAP.Get_BornYear() + "-" + VIDEOCAP.Get_BornMonth() + "-" + VIDEOCAP.Get_BournDay();

            //var img="data:image/jpeg;base64,"+VIDEOCAP.GetBase64Image(80);

            //return alert(VIDEOCAP.GetBase64Image(3));
            //$("#user-logo").attr("src",img);
