...
define(function (require) {
    ...
    app.controller('hRAdminSettingsCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.addStaff = function(){
            ...
            $scope.Staffs.addState = '3';
            $scope.Staffs.companyId = companyId;
        ...
        $scope.addTUser_permission = function(){
            ...
            $http.post(url + '/Admin/getStateByRole', {Staff: $scope.user_permisson}).success(function (data) {
                if(data == 1){
                    layer.confirm("该用户已经是系统管理员是否保留该身份？", {btn: ['是', '否']}, function () {
        ...
        $scope.getPremission = function(userId,id){
            ...
            //查询关联的功能权限 系统管理员
            $http.get(url + '/Admin/listAuthorityForPower/' + $scope.tUserPermissions.userId).success(function(data){
        ...
        $scope.setPermission = function(permissionsId){
            if(angular.isUndefined($scope.tUserPermissions.userId)){
                ...
                if (document.getElementById(permissionsId).checked == true) {
                    ...
                    $scope.tUserPermissions.tUserId = $scope.tUserPermissions.userId;
                    $http.post(url + '/Admin/deletePermissions' ,{HrAuthority:$scope.tUserPermissions}).success(function(data){
        ...
        $scope.setAuthority = function(corePositionId){
            ...
            if(angular.isUndefined($scope.hrAuthoritys.tUserId)){
                ...
                $http.get(url + '/Admin/ifOderToAuthorityByuserId/' + $scope.hrAuthoritys.tUserId + '/' + 'project').success(function(data){
                    if(data == 1){
                        ...
                        layer.alert('此管理员没有团队管理权限，请先分配团队管理权限',{icon : 0});
                        $scope.getPremission($scope.hrAuthoritys.tUserId,$scope.btnIndex_permission);
                    }
                }).error(function(data){
