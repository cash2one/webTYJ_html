...
define(function (require) {
    ...
    app.controller('staffSuperviseITCheckCtrl', ['$scope', '$http','$rootScope', '$location',function ($scope,$http,$rootScope,$location){

        $scope.doClick(7);//增加tab高亮显示 王洲 2016.2.2
        ...
        //得到在属性配置中的属性
        //学历
        $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/eduLever/' +$scope.projectId).success(function(data){
            $scope.eduLeverList = data.DataDictionarySlave;
        });
        //婚姻状况
        $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/married/' +$scope.projectId).success(function(data){
            $scope.marriedList = data.DataDictionarySlave;
        });
        //性别
        $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/sex/' +$scope.projectId).success(function(data){
            $scope.sexList = data.DataDictionarySlave;
        });
        //证件类型
        $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/cardType/' +$scope.projectId).success(function(data){
        ...
        $scope.toUpdate = function () {
            if ($scope.chooseData.datas.length > 1) {
                ...
                $scope.staff1 =$scope.chooseData.datas[0];
                $scope.staff1.birthday = new Date($scope.staff1.birthday );
                if($scope.staff1.cardNum.length>18){
                    $scope.cardNum=$scope.staff1.cardNum.substr(0, $scope.staff1.cardNum.indexOf('S'));
                }else{
                    $scope.cardNum=$scope.staff1.cardNum;
                }
       ...
       //移除图片
        ...
        $scope.save = function () {
            ...
            if(flag!=0)
                return;
            $scope.staff1.cardNum=$scope.cardNum;
