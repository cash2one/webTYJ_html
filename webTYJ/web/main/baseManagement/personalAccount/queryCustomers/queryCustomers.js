...
define(function (require) {
    ...
    app.controller('queryCustomersCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {

        var url = $rootScope.url;
        $scope.personCustNews={page:{}};     //人员信息
        $scope.buildingNew = {};            //建筑信息
        $scope.person={};
        $scope.personDatil=true;    //搜索显示页面
        $scope.showDatils=false;    //查看详情页面
        $scope.personUpdate=false;  //修改页面
        ...
        $scope.load=function(){
            var fetchFunction = function(page,callback) {
                ...
                $scope.personCustNews.companyId=userCook.companyId;
                $http.post(url + '/PersonCustNew/listPagePersonCustNewBySearch',{PersonCustNew:$scope.personCustNews}).success(callback);
            };
            $scope.btnIndex ='';
        ...
        $http.get(url + '/Project/getProjectByState').success(function(data){
            ...
            //婚姻状况
            $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/married/' +projectId).success(function(data){
                $scope.marriedList = data.DataDictionarySlave;
            });

            //性别
            $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/sex/' +projectId).success(function(data){
                $scope.sexList = data.DataDictionarySlave;
            });
            //证件类型
            $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/cardType/' +projectId).success(function(data){
        ...
        $scope.person={};
        //650bug 陈秋旭
        $scope.selectBox=function (item,event){
            console.log($scope.searchPaginator.object.personCustNew);
            for(var i=1;i<$scope.searchPaginator.object.personCustNew.length;i++){
               if(item.custId==$scope.searchPaginator.object.personCustNew[i].custId){
                   document.getElementById($scope.searchPaginator.object.personCustNew[i].custId).checked=true;
                   $scope.btnIndex =$scope.searchPaginator.object.personCustNew[i].custId;
                   $scope.d=true;
                   $scope.person=$scope.searchPaginator.object.personCustNew[i];
               }else{
                   document.getElementById($scope.searchPaginator.object.personCustNew[i].custId).checked=false;
               }
            }
            //bug 1617 陈秋旭
        ...
        $scope.show=function(item){
            ...
            $http.get(url + '/PersonCustNew/getPersonCustNewByIdRestful/'+$scope.custId).success(function(data) {
                $scope.person=data.PersonCustNew;
                console.log($scope.person);
                if($scope.person.annexs!=null){
                    if(angular.isArray($scope.person.annexs)){
                    }else{
                        $scope.annexList = [];
                        $scope.annexList.push($scope.person.annexs);
                        $scope.person.annexs= $scope.annexList;
                    }
                }
            }).error(function(data, status, headers, config){
                layer.msg("查询失败",{icon : 3,time : 2000});
            }) ;
            $scope.personDatil=false;    //搜索显示页面
            $scope.showDatils=true;    //查看详情页面
            $scope.personUpdate=false;  //修改页面
        };

        //查询所有的客户信息
        $scope.personCustNewList={};
        $http.get(url+'/PersonCustNew/listAllPersonCustNewRestful').success(function(data){
        ...
        $scope.checkValue=function(item){
            ...
            if(item.name ==null){
                ...
                layer.alert('性别不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.cardType ==null){
                layer.alert('证件类型不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.cardNum ==null|| item.cardNum==""){
                layer.alert('证件号码不能为空！',{icon : 0});
                flag=1;
                return;
            }else if(item.registerPhone ==null){
                layer.alert('注册电话不能为空！',{icon : 0});
                flag=1;
                return;
            }else {
                $scope.checkPhone(item.registerPhone);
                if(item.cardType ==0){
                    $scope.checkIdNumber(item.cardNum);
                }else{
                    $scope.checkIdNumber2(item.cardNum);
                }
                $scope.checkPhoneNum(3,item.urgentContactPhone);//紧急联系电话
                $scope.checkPhoneNum(0,item.phoneNum);//联系电话
                $scope.checkPhoneNum(1,item.phoneNum1);//联系电话1
                $scope.checkPhoneNum(2,item.phoneNum2);//联系电话2
                //if ($scope.person.cardNum != $scope.person.oldcardNum || $scope.person.registerPhone != $scope.person.oldregisterPhone) {
                //      if($scope.person.cardNum != $scope.person.oldcardNum){
                //          $http.post(url + '/PersonCustNew/getCountByNoOrPhone', {PersonCustNew: $scope.person}).success(function(data){
                //              if(data>0) {
                //                  layer.alert('您输入的证件号已存在！',{icon : 0});
                //                  return;
                //              }
                //
                //          })
                ...
                //      }
                //
                //    if($scope.person.registerPhone != $scope.person.oldregisterPhone){
                //        $scope.personCheck={};
                //        $scope.personCheck.registerPhone=$scope.person.registerPhone;
                //        $http.post(url + '/PersonCustNew/getCountByNoOrPhone', {PersonCustNew:  $scope.personCheck}).success(function(data){
                //            if(data>0) {
                //                $scope.checkUpdate = false;
                //                layer.alert('您输入的电话号码已被注册！',{icon : 0});
                //                return;
                //            }
                //        })
                //    }
        ...
        $scope.checkPhoneNum=function(number,phone){
            var pattern=/^((\+?86)|(\(\+86\)))?(13[012356789][0-9]{8}|15[012356789][0-9]{8}|18[02356789][0-9]{8}|147[0-9]{8}|1349[0-9]{7})$/;
            var patt=/^0\d{2,3}-?\d{7,8}$/;
            if(phone!=null&&phone!=""){
                if(!pattern.test(phone)&&!patt.test(phone)){
                    if(number=='0'){
                        ...
                        layer.alert('联系电话1格式不正确！',{icon : 0});
                        flag=1;
                        return;
                    }else if(number=='2'){
                        layer.alert('联系电话2格式不正确！',{icon : 0});
                        flag=1;
                        return;
                    }else if(number=='3'){
                        layer.alert('紧急联系电话格式不正确！',{icon : 0});
                        flag=1;
                        return;
                    }else{
                        layer.alert('注册电话格式不正确！',{icon : 0});
                        flag=1;
                        return;
                    }
        ...
        $scope.checkIdNumber=function(IdNumber){
            ...
            if(!pattern.test(IdNumber)){
                layer.alert('请输入正确的身份证号码！',{icon : 0});
                flag=1;
                return;
            }
        }
       ...
       //验证护照号
        $scope.checkIdNumber2=function(IdNumber){
            var pattern=/^1[45][0-9]{7}$|G[0-9]{8}$|P[0-9]{7}$|S[0-9]{7,8}$|D[0-9]$/;
            if(!pattern.test(IdNumber)){
                layer.alert('请输入正确的护照号！',{icon : 0});
                flag=1;
                return;
            }
        }
        ...
        $scope.getdata=function(items,event){
            ...
            for(var i=0;i<chk.length;i++){
                if( chk[i].checked==true){
                   $scope.person=items;
                    if($scope.person.cardNum.length>18){
                        $scope.person.cardNum=$scope.person.cardNum.substr(0, $scope.person.cardNum.indexOf('S'));
                    }else{
                        $scope.person.cardNum=$scope.person.cardNum;
                    }
        ...
        $scope.exportExcel=function() {
           var name=$scope.person.name;
            if (typeof(name) == "undefined") {
                ...
                var sexPerson=$('#sexPerson').val();
                var urgentContactPerson=$('#urgentContactPerson').val();
                var urgentContactPhone=$('#urgentContactPhone').val();
                var personName=$('#personName').val();


                selectUser="";
                exportExcelPerson1( name,cardType,cardNum,registerPhone,sexPerson,urgentContactPerson,urgentContactPhone);
            } else {
            selectUser = $scope.person.name;
            exportExcelPerson(selectUser);
        }
        }
        ...
        $scope.updateData=function(){
            if($scope.d==false){
                ...
                $scope.personDatil=false;    //搜索显示页面
                $scope.showDatils=false;    //查看详情页面
                $scope.personUpdate=true;  //修改页面
                $scope.person.birthday = new Date($scope.person.birthday);

                $scope.person.oldregisterPhone=$scope.person.registerPhone;
                ...
                $scope.person.oldcardNum=$scope.person.cardNum;
                ...
                }else if($scope.person.annexs ==undefined){
                    $scope.person.annexs=[];
                }
                //文件上传
                $("#zyupload").remove();
                ...
                $(function(){
                    ...
                    $("#zyupload").zyUpload({
                        ...
                        onSuccess: function(file, response){          // 文件上传成功的回调方法
                            ...
                            $scope.person.annexs.push($scope.annex);
        ...
        $scope.updateCustomer=function(item) {
            ...
            $scope.showDatils = false;    //查看详情页面
            $scope.personUpdate = true;  //修改页面
            $http.get(url + '/PersonCustNew/getPersonCustNewByIdRestful/' + item.custId).success(function (data) {
                $scope.person = data.PersonCustNew;
                $scope.person.birthday = new Date($scope.person.birthday);
        ...
        $scope.changeData=function(){
            var cardType = $("#cardsType").val();
            if(cardType=="number:1"){
                $scope.sf=false;
                $scope.hz=true;
            }else{
                $scope.person.cardNum='';
                $scope.sf=true;
                $scope.hz=false;
            }

            if(cardType=="number:0"){
                $scope.sf=true;
                $scope.hz=false;
            }else{
                $scope.person.cardNum='';
        ...
        $scope.updatePerson=function(){
            ...
            if ($scope.person.registerPhone.length==0) {

                layer.alert('请输入注册电话',{icon : 0});

                return false;
            }


            $scope.checkPhoneNum(4,$scope.person.registerPhone);//紧急联系电话
            $scope.checkPhoneNum(3,$scope.person.urgentContactPhone);//紧急联系电话
            $scope.checkPhoneNum(0,$scope.person.phoneNum);//联系电话
            $scope.checkPhoneNum(1,$scope.person.phoneNum1);//联系电话1
            $scope.checkPhoneNum(2,$scope.person.phoneNum2);//联系电话2

            if(flag!=0)
                return;
                $http.put(url+'/PersonCustNew/updatePersonCustNewRestful',{PersonCustNew:$scope.person}).success(function(data) {

                    var a=data.Info.info.$;
                    //a= a.split(",");
                    if (data.Info.info.$ == '编辑成功') {
                        layer.alert(data.Info.info.$,{icon : 0});
                        $scope.load();
                        $scope.d = false;
                        $scope.personDatil = true;
                        $scope.personUpdate = false;
                    }else {
                        layer.alert(data.Info.info.$,{icon : 0});
                        //layer.msg(a, {icon: 1, time: 2000});
                        //if (a[1].length > 0) {
                        //    layer.msg(a[1], {icon: 1, time: 2000});
                        //}
                        //for (var i = 0; i < a.length; i++) {
                        //    console.log(a[i]);
                        //    if (a[i].length > 0) {
                        //        layer.msg(a[i], {icon: 1, time: 2000});
                        //    }
                        //}
                    }
                }).error(function(data, status, headers, config){
        ...
        $scope.delPic = function(item){
            $scope.picItem = [];
            $scope.picItem =$scope.person.annexs ;
            $scope.person.annexs  = [];
            for(var i = 0; i <$scope.picItem.length; i ++){
                if(item.annexAddress != $scope.picItem[i].annexAddress){
                    $scope.person.annexs.push($scope.picItem[i]);
                }
            }
        };

        //修改取消
        $scope.cancle1Person=function(){
        ...
        $scope.addCity=function(){
            $('#chooseCity').modal({backdrop: 'static', keyboard: false});
        }
        //籍贯取消
        $scope.cancleCity=function(){
            $scope.person.nativePlace='';
        ...
        $scope.saveCity=function(){
            ...
            $scope.person.nativePlace=$scope.person.province +  $scope.person.city +  $scope.person.district;

            $('#chooseCity').modal('hide');
        }

        $scope.ByNoOrPhone=function(){
            $http.post(url + '/PersonCustNew/getCountByNoOrPhone', {PersonCustNew: $scope.person}).success(function(data){
                if(data>0){
                    $scope.checkUpdate=false;
                    return  data;
        ...
        $scope.finishPerson = function(){
            layer.confirm('是否完成个人客户？',
                {btn : ['确定','取消']},
                function(){
                    $http.post(url + '/projectCompletion/changeStateByProjectIdAndParamTypes/' + $scope.projectIds + '/' + 'person').success(function(data){
        ...
        $scope.showExcel=function(){
            ...
            angular.element("#remove").append("<div id='excelupload' class='zyupload'></div>");
            $(function(){
                // 初始化插件
                ...
                angular.element("#excelupload").zyUpload({
                    ...
                    onSuccess: function(file, response){          // 文件上传成功的回调方法
                        ...
                        $http.post(url + "/PersonCustNew/importExcelFile",{Annex:$scope.annex}).success(function(data){
                            if(data.Info.state){
                                ...
                                /*if(data.Info.state) {
                                    ...
                                    var filepath = sarr[3];
                                    if(e=='0'&&repeat=='0'){
                                        layer.msg("成功上传"+s+"条员工信息");
                                    }else{
                                        var index = layer.alert("成功上传" + s + "条员工信息，失败" + e + "条,重复"+repeat+"，点击确定下载错误信息", {
                                            closeBtn: 0
                                        }, function () {
                                            layer.close(index);
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data!=null && data!=undefined && data!=''){
                        ...
                        if(parseInt(uploadArr[2]) == parseInt(uploadArr[1])){
                            if(parseInt(uploadArr[0]) >= 0 &&parseInt(uploadArr[2])==parseInt(uploadArr[0])) {
                                layer.msg('导入成功！', {icon: 1});
                                angular.element("#uploadProcessDialog").modal('hide');
                                clearInterval(flagInterval);
                                $scope.load();
                            }else{
                                layer.msg('数据存在重复，请检查导入数据是否重复！', {icon: 0});
                                angular.element("#uploadProcessDialog").modal('hide');
                                clearInterval(flagInterval);
                                $scope.load();
                            }
         ...
         * 精拍仪
         */
        $scope.ReadIDCARDInfo = function(index) {

            console.log(VIDEOCAP);

            VIDEOCAP.ReadIdCard();

            if(VIDEOCAP.Get_IDNumber()==''){
                return layer.alert("未读取到相关证件",{icon : 2000});
            }

            $scope.person.cardNum = VIDEOCAP.Get_IDNumber();//身份证号码

            $scope.person.name = VIDEOCAP.Get_IDName();//姓名

            if(index==1){
                $scope.person.cardType = 0;
            }else{
                $scope.person.cardType = 1;
            }

            if(VIDEOCAP.Get_Sex()=='男'){
                $scope.person.sex = 0;
            }else{
                $scope.person.sex = 1;
            }
            //$scope.personCustNew.sex = VIDEOCAP.Get_Sex();

            $scope.person.nation = VIDEOCAP.Get_Nation()+'族';//民族

            $scope.person.birthday = VIDEOCAP.Get_BornYear() + "-" + VIDEOCAP.Get_BornMonth() + "-" + VIDEOCAP.Get_BournDay();

            //VIDEOCAP.Save_Photo("c:\\touxiang.jpg");  //保存身份证大头照
            //VIDEOCAP.SaveIdCard("c:\\idcard.jpg");  //保存身份证正反扫描件
        ...
        {"北京市": [
            ...
            "辽宁省": {"沈阳市": ["和平区", "沈河区", "大东区", "皇姑区", "铁西区", "苏家屯区", "东陵区", "新城子区", "于洪区", "辽中县", "康平县", "法库县", "新民市"],
...
function exportExcelPerson(personName){
    var url = document.getElementById("fileUrl").value;
    window.location.href=url +"/info/outPersonCustExcel.do?name="+personName;
    selectUser='';
}
function exportExcelPerson1( name,cardType,cardNum,registerPhone,sexPerson,urgentContactPerson,urgentContactPhone){
    var url = document.getElementById("fileUrl").value;
    window.location.href=url +"/info/outPersonCustExcel.do?name="+name+"&cardType="+cardType+"&cardNum="+cardNum+"&registerPhone="+registerPhone+"&sex="+sexPerson+"&urgentContactPerson="+urgentContactPerson+"&urgentContactPhone="+urgentContactPhone;;
    selectUser='';
}