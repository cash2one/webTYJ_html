...
define(function (require) {
    ...
    app.controller('inspectionOrdersCtrl', ['$scope', '$http','$rootScope','$stateParams','$location', function ($scope,$http,$rootScope,$stateParams,$location) {
        ...
        $scope.addSave=function(){
            ...
            $scope.currentEditArea={};
            $scope.editArea=false;
            $scope.RenovationInspectionOrders.renovationInspectionRecords = $scope.currentBuilding.areaNew;
        };
        /**
         * 新增取消
         */
...
//获取任务催促次数
        ...
        $http.get(url +'/TasksJournal/getTasksJournalbyTasksId/'+$scope.TasksJournal.taskId).success(function(data) {
            //alert("查询成功！");
            //console.log(data);
            $location.path("/index/serviceRequest/decorationManagement/inspectionOrders/"+$scope.TasksJournal.taskId).search({decorationWorkId:$scope.TasksJournal.taskId});
        ...
        $scope.item1=function(item){
            ...
            $http.get(url +'/TasksJournal/getTasksJournalbyDateandType/'+ item.creOrderType+"/"+item.operationTime).success(function(data) {
                ...
                //装修巡检单关闭显示
                if($scope.aa.renovationInspectionOrders!=null){
                    if($scope.aa.renovationInspectionOrders.orderState==0){
                        $scope.closed=true;
                    }else{
                        $scope.closed=false;
                    }
        ...
        $scope.RenovationInspectionOrders.letterRecords = $scope.currentLetter;
        $scope.RenovationInspectionOrders.taskId=$stateParams.decorationWorkId;
        $scope.RenovationInspectionOrders.renovationInspectionRecords = $scope.currentBuilding.areaNew;
        $scope.addCheckOrders = function(taskId){
            if($scope.Tasks.taskId!=0 || $scope.Tasks.taskId!=null){
                $http.post(url+'/RenovationInspectionOrders/insertRenovationInspectionOrders',{RenovationInspectionOrders:$scope.RenovationInspectionOrders}).success(function(){
                    $scope.RenovationInspectionOrders.taskId=$stateParams.decorationWorkId;
                    $location.path("/index/serviceRequest/decorationManagement/inspectionOrders/"+$scope.RenovationInspectionOrders.taskId).search({decorationWorkId:$scope.RenovationInspectionOrders.taskId});
                    $scope.load();
                }).error(function(data,status,headers,config){
                    alert("新增失败！");
                });
        ...
        $scope.CloseComplaintOrders=function(){
            ...
            $scope.aa.renovationInspectionOrders.closePersonId=15;
            $http.put(url+"/RenovationInspectionOrders/closeRenovationInspectionOrders",{RenovationInspectionOrders: $scope.aa.renovationInspectionOrders}).success(function(){
                //alert("关闭巡检状态成功！");
                $scope.closed=true;
            }).error(function(data,status,headers,config){
                alert("关闭巡检状态失败！");
            });
            $scope.aa.renovationInspectionOrders={}
        };

        $scope.fileState="";
        $scope.RenovationInspectionOrders.annexs=[];    //巡检单上传
        $scope.annex={
            annexAddress:'',
            annexName:'',
            relationId:''
