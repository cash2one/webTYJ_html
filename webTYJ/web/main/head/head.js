...
define(function (require) {
    ...
    app.controller('headCtrl', ['$scope','$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.personCustNew.companyId = $scope.user.companyId;
        ...
        $scope.editLogo = function(){
            ...
            if($scope.user.userId != '') {
                if ($scope.user.staff != null) {
                    $scope.tag = 0;//非超级管理员
                    layer.msg("您不是超级管理员，无法修改logo!", {icon: 0, time: 2000});
                } else {
                    $scope.tag = 1;//超级管理员
                }
            }
            if($scope.tag == 1) {
                ...
                //angular.element('#myModaleLogo').modal('toggle');
                $("#editlogo").remove();
                $("#edit").append("<div id='editlogo' class='zyupload'></div>");
                //上传图片
                $(function () {
                    // 初始化插件
                    angular.element("#editlogo").zyUpload({
                        width: "auto",                 // 宽度
                        height: "auto",                 // 宽度
