...
define(function (require) {
    var app = require('../../../../app');
    app.controller('appUserAddCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        **/
        ...
        $scope.checkValue=function(item){
            ...
            if((item.staffNo==null) || (item.staffNo=='')){
                ...
                flag=1;
                return;
            //}else if(item.birthday ==undefined) {
            //    layer.alert('出生日期格式不正确！', {icon: 0});
            //    flag = 1;
            //    return;
                ...
                layer.alert('员工姓名不能为空！',{icon : 0});
                flag=1;
                return;
            }/*else if((item.nativePlace ==null) || (item.nativePlace=='')){
                layer.alert('户籍不能为空！',{icon : 0});
                flag=1;
                return;
                ...
                flag=1;
                return;
            }else if($scope.checkPhone(item.telphone)==1){
                return;
            }
             /*else if((item.eduLever ==null)){
                layer.alert('学历不能为空！',{icon : 0});
                flag=1;
                return;
        ...
        $scope.insert=function(){
            if(isExistsPersonInWY)
                return layer.msg("用户已存在于物业管理平台！请核对后再添加！",{icon : 0,time : 2000});
            $scope.addStaff.companyId=$scope.user.companyId;

            if(!$scope.isExistsPerson){
                return layer.msg("用户不存在！请检查物业APP账户是否正确！",{icon : 0,time : 2000});
            }
            $scope.checkValue($scope.addStaff);
            $scope.addStaff.birthday = new Date($scope.addStaff.birthday);
            $scope.addStaff.processStatus = 1;

            if(flag!=0){
                return;
            }else{
                $http.post(url+'/staff/insertStaff',{Staff:$scope.addStaff}).success(function(){
        ...
        $scope.checkIsExistsPersonByAPPId = function(){
            ...
            if(loginId != undefined && loginId.replace(/(^s*)|(s*$)/g, "").length !==0){
                ...
                $http.post(url+'/staff/findUserInfoByUserAppId',{Staff:$scope.staffNews}).success(function(data){
                    ...
                    if(data.Staff.length > 0){
                        isExistsPersonInWY=true;
                        return layer.msg("用户已存在于物业管理平台！请核对后再添加！",{icon : 0,time : 2000});
                    }else{
                        isExistsPersonInWY=false;
                    }
                }).error(function(data, status, headers, config){
                ...
                $http.post(url+'/LoginService/checkUserByLoginNameAndSendMsg',{Staff:$scope.staffNews}).success(function(data){
                    console.log(data);
                });

                return;

                $http.get(url+'/LoginService/checkUserByAppAccountId/'+$scope.addStaff.userAppId).success(function(data){
        ...
        $scope.saveCity=function(){
            if ( $scope.addStaff.province != undefined || $scope.addStaff.city != undefined || $scope.addStaff.district != undefined) {
                $scope.addStaff.nativePlace = $scope.addStaff.province + $scope.addStaff.city + $scope.addStaff.district;
            }
            angular.element('#chooseCity').modal('hide');
        }
        ...
        $scope.NumberOrZm=function(staffNo) {
            var reg = /^(?!^\d+$)(?!^[a-zA-Z]+$)[0-9a-zA-Z]+$/;
            if (!reg.test(staffNo)||staffNo.length==6) {
                layer.alert('员工工号需由字母和数字组成且为6位！',{icon : 0});
            }
        }

        $scope.Zw=function(staffName) {
            var reg =/^[\u2E80-\u9FFF]+$/;

            if (!reg.test(staffName)) {
                layer.alert('员工姓名只能输入中文！',{icon : 0});
            }
        }
