...
define(function (require) {
    ...
    app.controller('AdvertisingLeaseCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.addProduct = function (){
            ...
            var pEffect = app.get('valueCheck').isNull($scope.AdvertisingLease.productBeginDate);
            if(!pEffect.state){
                layer.msg('请选择启用时间',{icon : 0,time : 1000});
                return;
            }
            var bdate=$scope.AdvertisingLease.productBeginDate;
            var edate=$scope.AdvertisingLease.productEndDate;
