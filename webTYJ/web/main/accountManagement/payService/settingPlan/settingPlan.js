...
define(function (require) {
    ...
    app.controller('settingPlanCtrl', ['$scope', '$http','$rootScope','$location', function ($scope,$http,$rootScope,$location) {
        ...
        $scope.searchProduct = {page: {}, projectId:$scope.project.projectId};
        $scope.btnIndex = 1;
        ...
        $http.get(url + '/Project/getProjectByState').success(function (data) {
            ...
            $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/startType/' + projectId).success(function (data) {
                ...
                if($scope.startType==null)
                {
                    layer.msg('获取启用状态失败,请刷新！',{icon : 0,time : 1000});
                }
            });
            $http.get(url + '/DataDictionarySlaveService/getDataDictionarySlaveRes/accountType/' + projectId).success(function (data) {
                ...
                {
                    layer.msg('获取账户类别失败,请刷新！',{icon : 0,time : 1000});
                }
            });
        });
        ...
        function chargetypeinit() {
            //设置优先级
            $http.get(url + '/ChargeType/getPriority/'+$scope.project.projectId).success(function (data) {
                $scope.priority = data.ChargeType;
                if($scope.priority==null)
                {
                    layer.msg('获取优先级失败,请刷新！',{icon : 0,time : 1000});
                }
            });
            //获取方案列表
        ...
        $scope.addChargeType = function (){
            ...
            $http.post(url + '/ChargeType/getCode').success(function (data) {
                ...
                $http.post(url + '/ProductCommonservice/getProductNumber/' + areaCode + '/' + projectId + '/' + divideCode + '/' + productTypeNumber + '/' + serialNumber).success(function (data) {
                    ...
                    if (app.get('valueCheck').isNull($scope.newchargeType.chargeTypeName).state) {
                        $http.get(url + '/ChargeType/checkNewName/'+$scope.newchargeType.chargeTypeName+'/'+$scope.project.projectId).success(function (data) {
                            if (data==1) {
                                layer.msg('此项目名称已存在，请重试！', {icon: 0, time: 1000});
                                return;
                            }else
                            {
        ...
        $scope.editPlan = function(){
            ...
            $scope.editChargeType=function(){
                ...
                if (app.get('valueCheck').isNull($scope.editchargeType.chargeTypeName).state) {
                    $http.get(url + '/ChargeType/checkEditName/'+$scope.editchargeType.chargeTypeName+'/'+$scope.editchargeType.chargeTypeId+'/'+$scope.project.projectId).success(function (data) {
                        if (data==1) {
                            layer.msg('此项目名称已存在，请重试！', {icon: 0, time: 1000});
                        }else
                        {
