 ...
 * 修改人：倪明   时间：2015/7/16.
 */
var chartMeterApp = angular.module('chartMeterApp',[]);
chartMeterApp.controller("meterChartCtrl",function($scope) {
    $scope.pageInfo={
        url:'http://localhost:8080/webTYJ/cxfws/rest/ElectricityMeterRecords/listElectricityMeterRecords',
        args:function(){
        ...
        getPageItems:function(data){
            return data['PageRestful']['electricityMeterRecords'];
        },
        pageStatus:{
            currentPage:1,
            showCount:3,
     ...
     */
    $scope.currentMeter = {};
    $scope.meterCheck = function(items){
        $scope.btnIndex = items;
        $scope.currentMeter = items;
        console.log($scope.currentMeter['initialReading']);
        var data1 = [[$scope.currentMeter['initialReading']],[40]];
        var data2 = [[70],[50]];
        var data3 = [[65],[45]];
